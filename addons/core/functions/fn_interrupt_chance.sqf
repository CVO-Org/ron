...
* their interruption chance (_chance_closestGroup) will be something like 0.6 * (vn_ron_interruption_enemy_inner_radius /_distance_ClostestGroup)
...
* something like (0.2 / x) * (vn_ron_interruption_enemy_inner_radius / _distance_x)
...
* _chance_closestGroup = (0.6 * (vn_ron_interruption_enemy_inner_radius /_distance_ClostestGroup)) + linearConversion [2, 8, _numberOfUnits, 0, 0.1, true];
* and the formula for other groups (calculated for each group, could be handled in the 'for' loop) would 

* the final chance of interruption (_chance_final) would be a sum of the _chance_closestGroup and every _chance_x element)( e_chance_cx= ((0.2 / x) * ((vn_ron_interruption_enemy_inner_radius / _distance_x)+ linearConversion [2, 8, _numberOfUnits, 0, 0.1, true];)
* 
*/
